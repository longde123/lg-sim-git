...
public class AgentManager : MonoBehaviour
{
    ...
    GameObject CreateSensor(GameObject agent, GameObject parent, GameObject prefab, JSONNode item)
    {
        ...
        try
        {
            foreach (var param in item["transform"])
            {
                var key = param.Key;
                var value = param.Value;

                String newKey = "configTransform" + key.ToUpper();

                var field = sbType.GetField(newKey);
                if (field != null)
                {
        ...
        {
            throw new Exception($"Failed to set ", ex);
        }

        try
        {
            foreach (var param in item["calibration"])
            {
                var key = param.Key;
                var value = param.Value;
                ...
                if (field != null)
                {
                    if (field.FieldType.IsGenericType && field.FieldType.GetGenericTypeDefinition() == typeof(List<>))
                    {
                        ...
                        if (type == typeof(double))
                        {
                            ...
                            {
                                Double elem = (Double)elemValue.Value.AsFloat;
                                list.Add(elem);
                            }
                        }

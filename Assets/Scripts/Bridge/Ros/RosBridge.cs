    ...
    public class Bridge : IBridge
    {
        ...
        public void SerializeInternal(object message, Type type, StringBuilder sb)
        {
            ...
            else if (BuiltinMessageTypes.ContainsKey(type))
            {
                ...
                {
                    if( message != null & (((float)message == 0) | ((float)message%1) == 0) ) {
                        sb.Append(string.Format(CultureInfo.InvariantCulture, "{0}.0", message));
                    }
                    else {
                    ...
                    if( message != null & (((Double)message == 0) | ((Double)message%1) == 0) ) {
                        sb.Append(string.Format(CultureInfo.InvariantCulture, "{0}.0", message));
                    }
                    else {
                        sb.Append(string.Format(CultureInfo.InvariantCulture, "{0}", message));
            ...
            {
                if (type.GetElementType() == typeof(byte) && (Version == 1 || Version == 2))
                {
                    sb.Append('"');

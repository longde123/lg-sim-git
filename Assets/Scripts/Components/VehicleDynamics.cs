...
public class VehicleDynamics : MonoBehaviour
{
    ...
    // Steering Parameters
    //
    [Tooltip("Tire steering range +/-")]
    public float maxSteeringAngle = 30.0f;

    [Tooltip("Steering Wheel to Tire Angle Ratio")]
    public float SteeringRatio = 16.0f;

    [Tooltip("Steering Tire Angle Rate (deg/s)")]
    public float maxSteeringRate = 30.0f;
    ...
    public void FixedUpdate()
    {
        ...
        {
            if(IgnitionStatus == IgnitionStatus.On && CurrentRPM > minRPM + (minRPM * 0.25))
            {
                creep_init = true;
            }
            //acceleration during creep behaviour
            if(vehicleController.BrakeInput == 0.0f && vehicleController.AccelInput == 0.0f && RB.velocity.magnitude <= 2.2f && creep_init == true)
            {
                vehicleController.AccelInput = 0.43f; 
            }

            SteerInput = Mathf.Clamp(vehicleController.SteerInput, -1.0f, 1.0f);
        ...
        foreach (var axle in axles)
        {
            if (axle.steering)
            {
                CurrentSteerAngle = axle.left.steerAngle;
                float maxSteerStep = maxSteeringRate * Time.fixedDeltaTime;

                float step = Mathf.Clamp(steer - CurrentSteerAngle, -maxSteerStep, maxSteerStep);
                float newAngle = Mathf.Clamp(CurrentSteerAngle + step, -maxSteeringAngle, maxSteeringAngle);

                //Debug.Log($"VD steer: {steer}, {CurrentSteerAngle}, {Time.fixedDeltaTime}, {maxSteerStep}, {step}, {newAngle},");

                axle.left.steerAngle = newAngle;
                axle.right.steerAngle = newAngle;
            }
        }

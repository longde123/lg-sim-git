...
public class NPCController : MonoBehaviour
{
    ...
    private int lane_index = 0;
    private bool isLaneSegmentFollow = false;
    ...
    private void GetNextLaneSegment()
    {
        // last index of current lane data
        if(lane_index < SimulatorManager.Instance.MapManager.trafficLaneSegments.Count - 1)
        {
            currentMapLane = SimulatorManager.Instance.MapManager.trafficLaneSegments[++lane_index];
            laneSpeedLimit = currentMapLane.speedLimit;
            aggressionAdjustRate = laneSpeedLimit / 11.176f; // 11.176 m/s corresponds to 25 mph
            normalSpeed = RandomGenerator.NextFloat(laneSpeedLimit - 3 + aggression, laneSpeedLimit + 1 + aggression);
            SetLaneData(currentMapLane.mapWorldPositions);
        ...
        {
            SimulatorManager.Instance.MapManager.trafficLaneSegments.Clear();
            lane_index = 0;
            ResetData();
            NPCManager.DespawnNPC(gameObject);
        }
    ...
    public void SetFollowLaneSegment(float maxSpeed, string[] lane_id_list)
    {
        isLaneSegmentFollow = true;
        Control = ControlType.FollowLane;

        var position = transform.position;

        var lane = SimulatorManager.Instance.MapManager.GetClosestLane(position, lane_id_list);
        InitLaneData(lane);
